...
def _style_ax(ax, title: str = "", xlabel: str = "", ylabel: str = "", grid_axis: str = "y"):
    if title:
        ax.set_title(title, fontsize=12, color="#102a43")
    if xlabel:
        ax.set_xlabel(xlabel, fontsize=10, color="#243b53")
    if ylabel:
        ax.set_ylabel(ylabel, fontsize=10, color="#243b53")
    ax.grid(True, axis=grid_axis, linestyle="--", linewidth=0.7, alpha=0.45)
...
with tab_data:
    if st.session_state["df"] is None:
        ...
        if df[tgt].nunique() <= 20:
            df[tgt].value_counts().plot(kind="bar", ax=ax, color="#4f46e5", width=0.75)
            ax.tick_params(axis="x", labelrotation=0)
        else:
            df[tgt].plot(kind="hist", bins=30, ax=ax, color="#4f46e5", edgecolor="#ffffff")
        _style_ax(ax, title=f"Target distribution: '{tgt}'", xlabel=tgt, ylabel="Count", grid_axis="y")
        _pyplot(fig)
...
with tab_train:
    if st.session_state["leaderboard"] is None:
        ...
        def _highlight(row):
            return [
                "background-color:#77fc81;color:#000000" if row["Best"] else "" 
                for _ in row
            ]
        ...
        colors = ["#10b981" if r["Best"] else "#6366f1" for r in lb_rows]
        ax.barh([r["Model"] for r in lb_rows[::-1]],
                [r[metric] for r in lb_rows[::-1]], color=colors[::-1], alpha=0.9)
        _style_ax(ax, title="Model leaderboard", xlabel=metric, grid_axis="x")
        _pyplot(fig)
        ...
        if valid_studies:
            ...
            if scores:
                fig, ax = plt.subplots(figsize=(7.2, 3.6))
                ax.plot(scores, alpha=0.35, color="#94a3b8", linewidth=1.2, label="trial")
                best_so_far = [max(scores[:i+1]) for i in range(len(scores))]
                ax.plot(best_so_far, color="#10b981", linewidth=2.2, label="best so far")
                _style_ax(ax, title=f"Optuna optimisation history: {sel}", xlabel="Trial", ylabel="Score")
                ax.legend()
                _pyplot(fig)
...
with tab_evaluate:
    if st.session_state["eval_result"] is None:
        ...
        if er["confusion_matrix"] is not None:
            st.subheader("Confusion matrix")
            cm = np.array(er["confusion_matrix"])
            fig, ax = plt.subplots(figsize=(min(8.5, cm.shape[0] + 2.6), min(6.5, cm.shape[0] + 1.6)))
            sns.heatmap(
                cm,
                annot=True,
                fmt="d",
            ...
            _style_ax(ax, title="Confusion matrix", xlabel="Predicted", ylabel="Actual", grid_axis="both")
            _pyplot(fig)
...
with tab_predict:
    if not st.session_state["pipeline_done"]:
        ...
        st.subheader("🔮 Make predictions on new data")
        method = st.radio("Input method", ["Upload CSV"], horizontal=True)
                ...
                try:
                    ...
                    if decision["problem_type"] == "classification":
                        try:
                            ...
                            ax2.barh([str(c) for c in classes], proba, color="#6366f1", alpha=0.9)
                            _style_ax(ax2, title="Prediction confidence", xlabel="Probability", grid_axis="x")
                            _pyplot(fig2)
                        except Exception:
                            pass
                except Exception as exc:
